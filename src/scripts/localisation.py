...
def produce_zoning(
    ...
    if buff_zones is not None:
        ...
        buff_zones = join_zones_to_bound(buff_gdf, buff_bound, "inside", buff_zones.id_col, buff_zones.name)
        output_gdfs.append(buff_zones)
        buff_int_bound = gpd.GeoDataFrame(pd.concat([int_bound, buff_bound]), geometry="geometry")
    else:
        buff_int_bound = int_bound

    ext_zones = join_zones_to_bound(ext_gdf, buff_int_bound, "outside", ext_zones.id_col, ext_zones.name)
    output_gdfs.append(ext_zones)

    int_zones = join_zones_to_bound(int_gdf, int_bound, "inside", int_zones.id_col, int_zones.name)
...
def main() -> None:
    ...
    with ctk.LogHelper(_NAME, details, log_file=log_file):
        ...
        LOG.info("Creating localisation zones for %s, with %s as the interal zoning system and %s as the external zoning system.",
        ...
        bound = bound_zones[bound_zones[parameters.localisation_area.selected_colname].isin(
            parameters.localisation_area.selected_lad
            )
        ]
        if parameters.zone_systems.buffer_zones is not None:
            LOG.info("Buffer zone system provided, will create buffer zones for boundary zones directly adjacent to internal boundary.")
        ...
        new_zones.to_file(
            parameters.output_folder / (f"zoning_localisation_{parameters.localisation_area.area_name}_"
                                        f"{parameters.zone_systems.internal_zones.name}.shp"),
            driver="ESRI Shapefile"
        )

        LOG.info("Finished creating localisation zones for %s. There are %s zones in the new zone system.",
                 parameters.localisation_area.area_name,
                 len(new_zones)
                 )
