...
class ZoneTranslation:
    ...
    def spatial_translation(self):
        ...
        with pd.ExcelWriter(
            ...
            missing_zones_1.to_excel(
                writer, sheet_name=f"{self.names[0]}_missing", index=False
            )
            missing_zones_2.to_excel(
                writer, sheet_name=f"{self.names[1]}_missing", index=False
            )
        LOG.info(
    ...
    def weighted_translation(self):
        ...
        weighted_translation.to_csv(self.cacher / f"{self.params.run_date}.csv", index=False)
        self.params.save_yaml(self.cacher / f"{self.params.run_date}.yml")
    ...
    def _slithers_and_rounding(self, translation: pd.DataFrame):
        if self.params.filter_slithers:
            ...
            ) = zc.find_slithers(translation, self.names, self.params.tolerance)
            ...
            if self.params.rounding:
                LOG.info("Checking all adjustment factors add to 1")
                final_zone_corr = zc.round_zone_correspondence(translation, self.names)
            else:
