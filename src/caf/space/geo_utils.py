...
def var_apply(
    ...
    area_correspondence_var: pd.DataFrame
        Zone to lower correspondence code with attached variable values scaled  by lower to zone correspondence values.
    """
    ...
    zone_variables[weighting_var_col] = zone_variables[weighting_var_col].astype(float)
...
def _cols_in_both(left: pd.DataFrame, right: pd.DataFrame):
    ...
    Also lowers all columns names to lower case to merge more flexibly.
    Args:
