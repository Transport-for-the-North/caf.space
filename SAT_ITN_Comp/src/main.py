...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Main module for running the ITN to SATURN correspondance analysis')
    parser.add_argument('itn', help='Path to the ITN shapefile', type=filePath)
    parser.add_argument('saturn', help='Path to the saturn link (or routes) shapefile', type=filePath)
    parser.add_argument('output', help='Path to the output csv file')
    parser.add_argument('--findlinks', default=False, action='store_true',
                        help='If given will process the inputs again even if the output csv exists')
    parser.add_argument('-z', '--zone_connectors', default=None, type=int, nargs=2,
                        help='Ignore nodes within this range, give a minimum and maximum value.')
    parser.add_argument('-w', '--weight_col', default=None, choices=['LENGTH', 'TIME'],
                        help='Column to use for the weights when calculating shortest path')
    parser.add_argument('--chunksize', type=int, default=None,
                        help='Amount of links in each processing chunk')
    parser.add_argument('--routes', default=False, action='store_true',
                        help='If given will preprocess the routes shapefile before finding the correspondance.')
    parser.add_argument('--processes', default=os.cpu_count()-1, type=int,
                        help='The number of processes to use when finding the link lookup.')
    parser.add_argument('--filter', default=False, action='store_true',
                        help='If given will filter the lookup shapefile based on the input before processing.')
    parser.add_argument('--filter_buffer', default=100, type=int,
                        help='Buffer around the input shapefile to be used for filtering the lookup shapefile.')
    # Read arguments
    ...
    print('\tDone in {} Total time {}'.format(timer.getLapTime(newLap=True), timer.getTimeTaken()))
    ...
    if kwargs['findLinks']:
        ...
        try:
            ...
            path = satLook.resultsPath[:satLook.resultsPath.rfind('.')] + '_Lengths.csv'
