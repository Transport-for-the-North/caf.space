...
def filterShapefile(inputShp, filterShp, buffer):
    ...
    filterShp = gpd.GeoDataFrame(
        {"geometry": filterShp.buffer(buffer)}, crs=filterShp.crs
    )
    ...
    subCols = [
        i for i in inCols if i != "geometry"
    ]  # Geometry column isn't hashable so can't check if it is identical for duplicates
...
def filter(inputPath, filterPath, buffer, outPath):
    ...
    # Filter shapefile
    print(
        f"Filtering shapefile, {len(inputShp):,} input links and {len(filterShp):,} filter links"
    )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Script for filtering the ITN network based on another network."
    )
    parser.add_argument("input", type=filePath, help="Path to input shapefile")
    parser.add_argument("filter", type=filePath, help="Path to the filter shapefile")
    parser.add_argument("buffer", type=int, help="Radius of buffer")
    # Get arguments
