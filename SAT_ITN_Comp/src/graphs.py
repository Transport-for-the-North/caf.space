...
class LengthGraph:
    ...
    DTYPES = {
        "SATURN_Link_ID": "str",
        "SATURN_Length(m)": "float",
        "ITN_Overlap_Length(m)": "float",
        "Length_Diff(m)": "float",
        "Length_%Diff": "float",
    }
    # Plotting constants
    ...
    def __init__(self, csv):
        ...
        if type(csv) is str:
            ...
            filePath(csv)
            self.df = pd.read_csv(
                csv, dtype=self.DTYPES, usecols=list(self.DTYPES.keys()), header=0
            )
    ...
    def plotHex(self, path=None):
        ...
        self.data = plt.hexbin(
            self.df["SATURN_Length(m)"],
            self.df["Length_%Diff"],
            gridsize=100,
            bins="log",
            mincnt=1,
            linewidths=0.1,
        ...
        if path is None:
            ...
            self.fig.canvas.mpl_connect(
                "motion_notify_event", lambda event: self.hover(event, "hexbin")
            )
            plt.show()
        else:
    ...
    def plotScatter(self, path=None):
        ...
        if path is None:
            # Show figure
            self.fig.canvas.mpl_connect(
                "motion_notify_event", lambda event: self.hover(event, "scatter")
            )
            plt.show()
        else:
            self.fig.savefig(self._checkPath(path), dpi=300)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Module for producing graphs for the SATURN to ITN script"
    )
    parser.add_argument(
        "-l",
        "--lengths",
