...
class CountLookup:
    ...
    def __init__(self, counts, links):
        ...
        # Rename columns
        self.countShp = self.countShp.rename(
            columns={v: k for k, v in self.COUNTS_COLS.items()}
        )
        self.linkShp = self.linkShp.rename(
            columns={v: k for k, v in self.LINKS_COLS.items()}
        )
    ...
    def calcDir(self):
        ...
        for i in (0, -1):
            ...
            coords = self.linkShp.geometry.apply(
                getCoords, args=(i,), pointType=False
            ).values
    ...
    def lookupCount(self, countRow):
        ...
        outDict = {
            "Count_ID": countRow["COUNT_ID"],
            "Count_Direction": countRow["DIRECTION"],
        }
        # Filter links
        links = self.linkShp[
            self.linkShp.intersects(countRow.geometry.buffer(self.COUNT_BUFFER))
        ]
        # Find closest line(s)
        ...
        if numFound == 1:
            ...
            outDict[
                "Comment"
            ] = f"ERROR: No link found within {self.COUNT_BUFFER} of count location."
            return outDict
    ...
    def loopCounts(self):
        ...
        self.lookup = pd.DataFrame(
            outLst,
            columns=["Count_ID", "Count_Direction", "Link_ID", "Link_Angle", "Comment"],
        )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description=(
            "Module to find the closest link for each count and "
