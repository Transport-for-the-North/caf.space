...
# Script modules
from utils import alphaShape, outputCsv, arrowCoords, calculateAngle, \
                    getCoords, pointToTuple, closestLine, overlapLength, \
                    getSingleLine, calcLengthDiff, TimeTaken
...
class SaturnItnLookup:
    ...
    ITN_BOUNDARY = ['concave', 1000]
    # Default ouput column names
    OUTPUT_COLS = ['SATURN_Link_ID', 'ITN_Link_ID', 'ITN_Overlap_Length(m)', 'ITN_SAT_Angle',
                    'Method', 'StartNode', 'EndNode', 'StartLink', 'EndLink',
                    'Comment']
    ...
    def __init__(self, satPath, itnPath, resultsPath, findLinks=True, **kwargs):
        ...
        # Print paths
        d = {'SATURN Shapefile':satPath, 'ITN Shapefile':itnPath, 'Output CSV':resultsPath}
    ...
    def updateVariables(self, **kwargs):
        ...
        def typeCheck(val, t):
            if not type(val) is t:
                raise TypeError('{} variable should be type {} but is {}'.format(
                                val, t, type(val)))
        ...
        # Variable dict
        varDict = {**dict.fromkeys(('buffers', 'saturn_input_cols', 'itn_input_cols'), upDict),
                    **dict.fromkeys(('itn_boundary', 'zone_connectors'), upList),
                    'weight_col':upStr, 'chunksize':upInt}
    ...
    def readShapefiles(self, satPath, itnPath, findLinks=True):
        ...
        # If link ID column present don't need A/B node
        if not self.SATURN_INPUT_COLS['LINK_ID'] is None:
            for i in ('Anode', 'Bnode'):
                self.SATURN_INPUT_COLS[i] = None

        # Check if columns required are present
        for shp, cols, path in (('itnNet', 'ITN_INPUT_COLS', itnPath), ('satNet', 'SATURN_INPUT_COLS', satPath)):
            for k, val in getattr(self, cols).items():
                if val is None: continue
                if val not in getattr(self, shp).columns.tolist():
                    raise KeyError('Column containing {} data named {} is not present in {}'.format(k, val, path))
        ...
        if self.SATURN_INPUT_COLS['LINK_ID'] is None:
            self.satNet['LINK_ID'] = self.satNet.Anode.astype(str) + '_' + self.satNet.Bnode.astype(str)
        ...
        for key, val in {'satNet':'SATURN_INPUT_COLS', 'itnNet':'ITN_INPUT_COLS'}.items():
            dups = getattr(self, key)['LINK_ID'].duplicated().sum()
            idCol = getattr(self, val)['LINK_ID']
            if dups > 0:
                raise ValueError(f"{dups:,} duplicated LINK_IDs found in {idCol} for {key}")
        ...
        if findLinks:
            ...
            try:
                ...
                self.itnNet[self.WEIGHT_COL] = self.itnNet[self.WEIGHT_COL].astype(float)
            ...
            self.itnGraph.add_weighted_edges_from(self.itnNet[['Vertices_Tup', 'Vertices2_Tup', 'WEIGHT']].values)
    ...
    def createBoundaries(self):
        ...
        print('\tCreating boundary using {} with -{}m buffer.'.format(*self.ITN_BOUNDARY))
        ...
        if self.ITN_BOUNDARY[0] == 'convex':
            ...
            if not self.ITN_BOUNDARY[0].lower().endswith('.shp'):
                raise IOError('File given is not a shapefile, {}'.format(self.ITN_BOUNDARY[0]))
            # Read shapefile
            shp = gpd.read_file(self.ITN_BOUNDARY[0])
            # Check if there is only 1 feature
            if len(shp) != 1:
                raise ValueError('Shapefile {} has {} features expected 1'.format(self.ITN_BOUNDARY[0], len(shp)))
            # Check if it is a polygon
            shp = shp.geometry.iloc[0]
            if not type(shp) is Polygon:
                raise TypeError('Shapefile {} has feature {} expected Polygon'.format(self.ITN_BOUNDARY[0], type(shp)))
            # Use provided shapefile as boundary
            self.itnBoundary = shp
        else:
            raise ValueError("ITN_BOUNDARY should be 'convex', 'concave' or the path to a shapefile with a single polygon.")

        # Get area close to edge of ITN layer
        self.itnEdgeArea = self.itnBoundary.difference(self.itnBoundary.buffer(-1 * self.ITN_BOUNDARY[1]))

        # Change matplotlib backend as it was causing an issue with multiprocessing
        mpl.use('agg')
        # Create the figure
        fig, ax = plt.subplots()
        plt.subplots_adjust(top=0.95, bottom=0.01, right=0.99, left=0.01, hspace=0, wspace=0)
        # Set axis paramters
        ...
        linCol = LineCollection(linCol, label='ITN Links', color='black', linewidth=0.1, zorder=0)
        ax.add_collection(linCol)
        # Plot boundaries
        ax.fill(self.itnBoundary.exterior.xy[0], self.itnBoundary.exterior.xy[1], color='blue', alpha=0.5, label='ITN Area', zorder=5)
        itnEdge = self.itnEdgeArea
        ax.fill(itnEdge.exterior.xy[0] + itnEdge.interiors[0].xy[0], itnEdge.exterior.xy[1] + itnEdge.interiors[0].xy[1],
                    alpha=0.5, color='red', label='ITN Edge ({}m)'.format(self.ITN_BOUNDARY[1]), zorder=10)
    ...
    def calcAngle(satLink, itnLink):
        ...
        try:
            ...
            err = f'Model Link: {satLink.LINK_ID}, Lookup Link: {itnLink.LINK_ID}. ' + str(e)
    ...
    def findItnNode(self, satNode, itnLinks):
        ...
        # Find nodes
        itnNodes = pd.Series(itnLinks.Vertices.append(itnLinks.Vertices2)
                                        .apply(pointToTuple).unique())
    ...
    def checkItnEdge(self, satLink):
        ...
        if satLink.geometry.within(self.itnBoundary):
            ...
            return 'WARNING: SATURN link is within {}m of the edge of the ITN layer'.format(self.ITN_BOUNDARY[1])
        else:
            return ''
    ...
    def findItnLinks(self, satLink):
        ...
        try:
            ...
            if not satLink.geometry.intersects(self.itnBoundary):
                outLst.append({'SATURN_Link_ID':satLink.LINK_ID, 'Comment':'ERROR: SATURN link outside of ITN area'})
                return satLink.LINK_ID, outLst

            # Get list of ITN links that intersect the SATURN link
            ins = self.itnNet.intersects(satLink.geometry.buffer(self.BUFFERS['itnFilter']))
            ...
            for iItn, itnLink in itnIns.iterrows():
                ...
                if (satLink.geometry.within(itnLink.geometry.buffer(self.BUFFERS['withinRound']))
                    or satLink.geometry.within(itnLink.geometry.buffer(self.BUFFERS['withinFlat'], cap_style=2))):
                    # Add to list
            ...
            if length == 1:
                ...
                row = {'SATURN_Link_ID':satLink.LINK_ID, 'ITN_Link_ID':linksWithin[0].LINK_ID,
                        'Method':method, 'ITN_Overlap_Length(m)':self.overlapLen(satLink, linksWithin[0]),
                        'ITN_SAT_Angle':self.calcAngle(satLink, linksWithin[0]), 'Comment':self.checkItnEdge(satLink)}
                # Add data to outLst
                outLst.append(row)
            elif length >= 2:
                try:
                    ...
                    # Create dict for adding row
                    row = {'SATURN_Link_ID':satLink.LINK_ID, 'ITN_Link_ID':itnLink.LINK_ID,
                            'Method':method, 'ITN_Overlap_Length(m)':self.overlapLen(satLink, itnLink),
                            'ITN_SAT_Angle':self.calcAngle(satLink, itnLink), 'Comment':self.checkItnEdge(satLink)}
                except ValueError as e:
                    # Add error data to row
                    row = {'SATURN_Link_ID':satLink.LINK_ID, 'Method':method, 'Comment':'AngleError: {}'.format(e)}
                # Add row to list
                outLst.append(row)
                    ...
                    if not n is None:
                        ...
                        if num == 1:
                            ...
                            try:
                                ...
                                itnLink = self.compareDirections(satLink, (closest.iloc[0], closest.iloc[1]))
                ...
                tmpGraph.add_weighted_edges_from(itnIns[['Vertices_Tup', 'Vertices2_Tup', self.WEIGHT_COL]].values)
                ...
                try:
                    ...
                    # Find shortest path
                    path = nx.shortest_path(tmpGraph, nodes[0], nodes[1], weight='weight')
                except nx.NetworkXNoPath as e:
                    comment = 'NoPathError: No path in the ITN network found.'
                    row = {'SATURN_Link_ID':satLink.LINK_ID, 'Method':method, 'StartNode':nodes[0],
                            'EndNode':nodes[1], 'StartLink':links[0], 'EndLink':links[1],
                            'Comment':'{} {}'.format(comment, self.checkItnEdge(satLink))}
                    outLst.append(row)
                except IndexError as e:
                    comment = 'ERROR: {}'.format(e)
                    row = {'SATURN_Link_ID':satLink.LINK_ID, 'Method':method,
                            'Comment':'{} {}'.format(comment, self.checkItnEdge(satLink))}
                ...
                for n in range(len(path)-1):
                    itnLink = self.itnNet[(self.itnNet.Vertices_Tup==path[n]) & (self.itnNet.Vertices2_Tup==path[n+1])]
                    ...
                    # Create row of data
                    row = {'SATURN_Link_ID':satLink.LINK_ID, 'ITN_Link_ID':itnLink.LINK_ID,
                            'Method':method, 'StartNode':nodes[0], 'EndNode':nodes[1],
                            'StartLink':links[0], 'EndLink':links[1],'ITN_SAT_Angle':angle,
                            'Comment':comment}
    ...
    def loopSatLinks(self, processes=1):
        ...
        if self.SATURN_INPUT_COLS['LINK_ID'] is None:
            ...
            self.satNet = self.satNet[((self.satNet.Anode < self.ZONE_CONNECTORS[0]) |
        ...
        loopTimer = TimeTaken()
        outputLst = []
        print('Looping through all ({:,}) of the links in the SATURN network'.format(tot))
        ...
        try:
            if processes > 1:
                ...
                # Process chunks
                print('Using {} processes with a chunksize of {}'.format(processes, chunkSize))
                pbar = tqdm(desc='Processing Links', total=tot, unit=' links')
                for res in pool.imap_unordered(self.findItnLinks, self.satNet.iterrows(), chunksize=chunkSize):
                    count += 1
                    if count >= chunkSize:
                ...
                for iSat, satLink in tqdm(self.satNet.iterrows(), desc='Processing Links', unit=' links',
                                          total=tot):
                    # Find itn links
            ...
            self.outputDf = pd.DataFrame(columns=self.OUTPUT_COLS,
                                            data=outputLst)
    ...
    def compareLengths(self):
        ...
        self.outputDf = self.outputDf.merge(self.satNet[['LINK_ID', 'LENGTH']], left_on='SATURN_Link_ID',
                                            right_on='LINK_ID', how='left', validate='many_to_one')
        self.outputDf = self.outputDf.drop('LINK_ID', axis=1).rename(columns={'LENGTH':'SATURN_Length(m)'})
        # Reorganise columns
        ...
        lengthComp = calcLengthDiff(self.outputDf[[groupCol, *lengthCols.keys()]],
                                    groupCol, lengthCols)
    ...
    def plotLinks(self, satLinkId):
        ...
        if satLinkId not in self.outputDf.SATURN_Link_ID.tolist():
            ...
            return
        # Get saturn link
        satLink = self.satNet.loc[self.satNet.LINK_ID==satLinkId, ['geometry', 'LENGTH']].iloc[0]
        # Convert to LineString
        satLink.loc['geometry'] = getSingleLine(satLink.geometry)
        # Get corresponding ITN links and the overlap length
        itnLinks = self.outputDf.loc[self.outputDf.SATURN_Link_ID==satLinkId].reset_index(drop=True)
        ...
        ax.arrow(x, y, dx, dy, color=color, ec=color, head_width=hw, overhang=0,
                    head_length=hl, length_includes_head=True, zorder=0)
        ax.plot(satLink.geometry.xy[0], satLink.geometry.xy[1], color, linewidth=5,
                label='Sat Link: {}, {:0.1f}m'.format(satLinkId, satLink['LENGTH']), zorder=0)

        # Plot each ITN link
        count = 0
        for i, link in itnLinks.iterrows():
            if not (link.Comment is None or link.Comment is np.nan or link.Comment == ''):
                print(link.Comment)
            # Check if ITN link column is not blank
            if link.ITN_Link_ID is None or link.ITN_Link_ID is np.nan or link.ITN_Link_ID == '':
                continue
            # Get itn link
            try:
                itnLink = self.itnNet.loc[self.itnNet.LINK_ID==link.ITN_Link_ID, 'geometry'].iloc[0]
            ...
            ax.arrow(x, y, dx, dy, color=color, ec=color, head_width=hw, overhang=1.0,
                    head_length=hl, length_includes_head=True, zorder=5)
            ax.plot(itnLink.xy[0], itnLink.xy[1], color, zorder=5,
                        label='ITN: {}, {:0.1f}m'.format(link.ITN_Link_ID, link['ITN_Overlap_Length(m)']))
            ...
            if itnLinks.iloc[0]['Method'].startswith('ShortestPath'):
                for i in ('StartNode', 'EndNode'):
                    ...
                    if not type(tup) == tuple or len(tup) != 2:
                        break
                    ax.scatter(tup[0], tup[1], color='black', marker=mark, label=i,
                                zorder=10)
