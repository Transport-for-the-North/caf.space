...
from shapely.geometry import MultiPoint, MultiLineString, MultiPolygon, LineString, Point
...
class Loggers:
    ...
    def __init__(self, logFile):
        ...
        # Create formatter
        format = logging.Formatter("%(asctime)s [%(name)-20.20s] [%(levelname)-8.8s] %(message)s")
        fh.setFormatter(format)
        ch.setFormatter(logging.Formatter("[%(levelname)-8.8s] %(message)s"))

        # Create queue listener
        self.listener = logging.handlers.QueueListener(self.QUEUE, fh, ch, respect_handler_level=True)
...
def calculateAngle(line1, line2, directed=False):
    ...
    with np.errstate(all='raise'):
        try:
            ...
            print(e, 'vector 1 {}, vector 2 {}, dot {}, normalised vectors {}'.format(
...
def getSingleLine(line):
    ...
    if isinstance(line, LineString):
        ...
        if isinstance(line, LineString):
            ...
            warn(('MultiLineString converted to LineString using list method '
